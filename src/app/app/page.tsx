...
export default function Home() {
  ...
  return (
    <main className="h-[100dvh] w-full bg-black text-white flex flex-col overflow-hidden">
      ...
      <header className="w-full flex items-center justify-between p-6 shrink-0">
        ...
        <button 
          ...
          className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center border border-white/5 hover:bg-zinc-800 transition active:scale-95 cursor-pointer shrink-0"
        >
          <Settings className="w-5 h-5 text-zinc-400" />
        </button>
      </header>

      <div className="flex-1 w-full flex flex-col overflow-y-auto pb-[calc(5rem+env(safe-area-inset-bottom))]">
        <TeeziView />
      </div>
